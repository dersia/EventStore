...
namespace EventStore.Core.Tests.Index.IndexV1 {
	...
	public class when_a_ptable_header_is_corrupt_on_disk : SpecificationWithDirectory {
		...
		public void the_hash_is_invalid() {
			var exc = Assert.Throws<CorruptIndexException>(() => _table = PTable.FromFile(_copiedfilename, ESConsts.PTableInitialReaderCount, TFChunkHelper.PTableMaxReaderCountDefault, 16, false));
		...
		public void no_error_if_index_verification_disabled() {
			Assert.DoesNotThrow(
				() => _table = PTable.FromFile(_copiedfilename, ESConsts.PTableInitialReaderCount, TFChunkHelper.PTableMaxReaderCountDefault, 16, true)
			);
		}
