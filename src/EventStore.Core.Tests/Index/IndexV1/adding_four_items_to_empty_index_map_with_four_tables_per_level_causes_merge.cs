	...
	public class
		...
		public override void TestFixtureSetUp() {
			...
			_result = _map.AddPTable(
				PTable.FromMemtable(memtable, GetTempFilePath(), ESConsts.PTableInitialReaderCount, TFChunkHelper.PTableMaxReaderCountDefault, skipIndexVerify: _skipIndexVerify), 1, 2,
				(streamId, hash) => hash,
				_ => true,
			...
			_result = _result.MergedMap.AddPTable(
				PTable.FromMemtable(memtable, GetTempFilePath(), ESConsts.PTableInitialReaderCount, TFChunkHelper.PTableMaxReaderCountDefault, skipIndexVerify: _skipIndexVerify), 3, 4,
				(streamId, hash) => hash,
			...
			_result = _result.MergedMap.AddPTable(
				PTable.FromMemtable(memtable, GetTempFilePath(), ESConsts.PTableInitialReaderCount, TFChunkHelper.PTableMaxReaderCountDefault, skipIndexVerify: _skipIndexVerify), 4, 5,
				(streamId, hash) => hash,
				_ => true, _ => new System.Tuple<string, bool>("", true), new GuidFilenameProvider(PathName),
				_ptableVersion, _maxAutoMergeIndexLevel, 0, skipIndexVerify: _skipIndexVerify);
			_result.ToDelete.ForEach(x => x.MarkForDestruction());
			...
			_result = _result.MergedMap.AddPTable(
				PTable.FromMemtable(memtable, GetTempFilePath(), ESConsts.PTableInitialReaderCount, TFChunkHelper.PTableMaxReaderCountDefault, skipIndexVerify: _skipIndexVerify), 0, 1,
				(streamId, hash) => hash,
