...
namespace EventStore.Projections.Core.Tests.Services.command_reader_response_reader_integration {
	...
	public class
		...
		private Guid _runCorrelationId = Guid.NewGuid();

		protected override void Given() {
			_numberOfWorkers = 1;
			var controlStream = ProjectionNamesBuilder.BuildControlStreamName(_runCorrelationId);
		...
		protected override IEnumerable<WhenStep> When() {
			yield return new WhenStep(
				new ProjectionCoreServiceMessage.StartCore(_runCorrelationId),
				new ProjectionManagementMessage.Starting(_runCorrelationId));
		}
