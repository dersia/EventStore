	...
	public class when_the_subsystem_components_start : TestFixtureWithProjectionCoreAndManagementServices {
		...
		public void readers_should_use_the_same_control_stream_id() {
			Assert.AreEqual(_uniqueId,
				_consumer.HandledMessages.OfType<ProjectionManagementMessage.Starting>().First().RunCorrelationId);
			Assert.AreEqual(_uniqueId,
				_consumer.HandledMessages.OfType<ProjectionCoreServiceMessage.StartCore>().First().RunCorrelationId);
		}
	}
}
